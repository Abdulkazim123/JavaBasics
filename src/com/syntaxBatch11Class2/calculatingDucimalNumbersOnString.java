package com.syntaxBatch11Class2;

public class calculatingDucimalNumbersOnString {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

		
		double number1=10.50;
		double number2=10.50;
		
		 double sum= number1+number2;
		 double sub= number1-number2;
		 double mul= number1*number2;
		 double div= number1/number2;
		
		 System.out.println(" The addition of 2 numbers"+number1+"and"+number2+"is equal to "+sum);
		
		 System.out.println("The subtration of 2 numbers"+number1+"and"+number2+"is equal to"+sub);
		
		 System.out.println("The mul of 2 numbers"+number1+"and"+number2+" is equal to"+mul);
		 
		 System.out.println(" The division of 2 numbers"+number1+"and"+number2+"is equal to "+div);
		 
		 
		 
		
	}

}
